...
describe("tryToCreateGroup", () => {
    it("should fail when group name is taken", async () => {
        await expect(tryToCreateGroup(KTEST_GROUP_NAME, "White", KTEST1_UID)).rejects.toThrow(CREATE_GROUP_ERROR_CODES.GROUP_NAME_TAKEN)
    });

    it("success case", async () => {
        let groupName = "JestCreateGroupTest";
        const groupCode = await tryToCreateGroup(groupName, "White", KTEST1_UID);
