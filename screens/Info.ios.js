  ...
  return (
    <SafeAreaView style={styles(theme).container}>
      <View style={styles(theme).buttonContainer}>
        <TouchableOpacity style={styles(theme).button} onPress={() => pageViewRef.current.setPage(0)}>
          <Text style={styles(theme).buttonText}>Hours</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles(theme).button} onPress={() => pageViewRef.current.setPage(1)}>
          <Text style={styles(theme).buttonText}>Tent Checks</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles(theme).button} onPress={() => pageViewRef.current.setPage(2)}>
          <Text style={styles(theme).buttonText}>Missed Checks</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles(theme).button} onPress={() => pageViewRef.current.setPage(3)}>
          <Text style={styles(theme).buttonText}>Grace</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles(theme).button} onPress={() => pageViewRef.current.setPage(4)}>
          <Text style={styles(theme).buttonText}>Tent Types</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles(theme).button} onPress={() => pageViewRef.current.setPage(5)}>
          <Text style={styles(theme).buttonText}>P-Checks</Text>
        </TouchableOpacity>
      </View>

      <PagerView style={{ flex: 1 }} initialPage={0} ref={pageViewRef}>
        <View key='0'>
          <ScrollView style={{ marginHorizontal: 20, marginTop: 10 }} showsVerticalScrollIndicator={false}>
            ...
            <Text style={styles(theme).contentText}>
              A tent is comprised of a maximum of 12 people. Occupancy requirements for each tenting period remain the
              same regardless of the number of people in the tent.
            </Text>

            <Text style={styles(theme).header}>Daytime Hours and Curfew</Text>
            <Text style={styles(theme).contentText}>
              Tenters on duty must be within the K-Ville city limits at all times. The number of tenters required at a
              given time varies based on the current tenting period and the time of day.
            </Text>

            <Text style={styles(theme).contentText}>
              Each day is divided into day and night hours. A different number of tenters on duty is necessary from day
              to night.
            </Text>
        ...
        <View key='1'>
          <ScrollView style={{ marginHorizontal: 20, marginTop: 10 }} showsVerticalScrollIndicator={false}>
            <Text style={styles(theme).header}>Tent Checks</Text>
            <Text style={styles(theme).contentText}>
              The Line Monitors may announce a tent check at their discretion. A check will be signaled by the sounding
              of a bullhorn siren.
            </Text>

            <Text style={styles(theme).contentText}>
              To check in, gather all members of your tent in the area between the Card parking lot and the
              Schwartz-Butters plaza. Wait until the required number of tenters are present to check in — we cannot
              check in your tent until all required members are present.
            </Text>

            <Text style={styles(theme).contentText}>
              Tents who have yet to be checked but appear to be missing will — at a minimum — be given three warning
              calls over the bullhorn before being marked as absent. After the final warning call, an additional 2
              minutes will be given to allow for tenters to check in. After this time elapses, the check is officially
              over.
            </Text>
            ...
            <Text style={styles(theme).contentText}>
              Line Monitors cannot be held responsible for checks missed due to tenters using the bathroom, failing to
              hear the siren, being asleep in the tent, or similar related circumstances.
            </Text>
          </ScrollView>
        </View>
        ...
        <View key='4'>
          <ScrollView style={{ marginHorizontal: 20, marginTop: 10 }} showsVerticalScrollIndicator={false}>
            ...
            <Text style={styles(theme).contentText}>
              Flex Tenting abides by the same rules and regulations as White Tenting, but Flex tents are NOT GUARANTEED
              a spot in the Carolina game.
            </Text>
            <Text style={styles(theme).contentText}>
              These tenting spots will be filled through the Race to the Secret Spots. The 31st to 60th groups to
              complete the Race to the Secret Spots will receive Flex Tenting spots.
            </Text>
            <Text style={styles(theme).contentText}>
